...
straligns = ("right", 'left', 'center')
numaligns = ("right", "center", "left", "decimal")
HEADERS = ("firstrow", "keys")
showindex = ("always", "never", True, False, 'iterable')

COL_S_RE = regex.compile('\s*\|\s*', regex.V1)
missingval = '$'
MISSING_ROW = regex.compile('(\|)\s*(\$)', regex.V1)
...
rst = [('lineabove', Line(begin='', hline='=', sep='  ', end='')),
...
pipe = [('lineabove', tabulate._pipe_line_with_colons),
('linebelowheader', tabulate._pipe_line_with_colons),
('linebetweenrows', None),
('linebelow', None),
('headerrow', DataRow(begin='|', sep='|', end='|')),
('datarow', DataRow(begin='|', sep='|', end='|')),
...
psql = [('lineabove', Line(begin='+', hline='-', sep='+', end='+')),
        ('linebelowheader', Line(begin='|', hline='-', sep='+', end='|')),
        ('linebetweenrows', None),
        ('linebelow', Line(begin='+', hline='-', sep='+', end='+')),
        ('headerrow', DataRow(begin='|', sep='|', end='|')),
...
grid = [('lineabove', Line(begin='+', hline='-', sep='+', end='+')),
        ('linebelowheader', Line(begin='+', hline='=', sep='+', end='+')),
...
+------------+------------+-----------+
'''
ex_row = ['body row 1', 'column 2', 'column 3 ']
ex_span_col = ['body row 2', 'Cells may span columns.']

#tabulate([c,b], tablefmt='grid', missingval='$')





def tabulate(tabular_data, headers=(), tablefmt="rst",
	...
	cols = [[_format(v, ct, floatfmt, missingval, has_invisible) for v in c] for c, ct in zip(cols, coltypes)]
	...
	if headers:
		...
		headers = [_align_header(h, a, minw, width_fn(h)) for h, a, minw in zip(headers, t_aligns, minwidths)]
		rows = list(zip(*cols))
	else:
	...
	if not isinstance(tablefmt, TableFormat):
		tablefmt = _table_formats.get(tablefmt, _table_formats["rst"])
